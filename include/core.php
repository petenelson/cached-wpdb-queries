...
function get_cached_queries() {
	// See queries/get-available-post-mime-types.php for an example.
 ...
 * @return string
 */
function update_wpdb_query( $query ) {
 ...
 */
function maybe_use_cached_results( $query_data ) {
	...
	if ( false === $option || ! is_array( $option ) ) {
		...
		if ( time() > $option['expires'] ) {
			...
			// This has not expired yet. We're good to use the updated wpdb query.
			$rebuild           = false;
